...
<template>
    <input type="number" maxLength="5" placeholder="Wpisz kod" class="search" v-model="searchQuery"
        @input="event => { if (event.target.value.length > event.target.maxLength) searchQuery = event.target.value.slice(0, event.target.maxLength); }" />
    <button class="button success" @click="search"
        v-if="searchQuery != '' && !hideScanner">Sprawdź</button>
