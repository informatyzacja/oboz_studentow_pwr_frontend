...
<template>
    <TopBar title="FAQ" :backLink="$router.options.history.state.back.substring(0, 4) != '/faq' ? $router.options.history.state.back : '/profil'"/>
    <div v-if="apiDataStore.faq.ready" class="padding">
        <RouterLink :to="'/faq/'+data.id" v-for="(data, index) in apiDataStore.faq.data" :key="index">
            <ItemBox :bigText="data.question" :smallText="data.answer.length > 40+3 ? data.answer.substring(0, 40) + '...' : data.answer" rightIcon="arrow.svg"/>
        </RouterLink>
    </div>
