...
<template>
  <main>
  <TopBar :title="apiDataStore.profile.ready ? ('Domek nr ' + apiDataStore.profile.data[0].house.name) : 'Mój domek'" backLink="/czat-domku"/>

  <div class="padding-main">

    <h3>Informacje</h3>
    <ItemBox :bigText="apiDataStore.profile.data[0].house.key_collected ? 'Klucze odebrane ✅' : 'Klucze nieodebrane ❌' "></ItemBox>

    <h3>Lokatorzy</h3>
    <div
      v-if="
        apiDataStore.myHouseMembers.ready &&
        apiDataStore.myHouseMembers.data &&
        apiDataStore.myHouseMembers.data.length
      "
    >
      <a
        v-for="(data, index) in apiDataStore.myHouseMembers.data"
        :key="index"
